module JsonBuilder
  ...
  def self.json_builder(json, status, msg, *args)
    ...
    info[:next_page_exist]        = options[:next_page_exist] if options[:poll_list].present?
    info[:previous_page_exist]    = options[:previous_page_exist] if options[:poll_list].present?
